...
const instrumentPage = (url) => {
    ...
    return new Promise((resolve) => {
        ...
        const lighthouse = child_process_1.spawn(lighthousePath, args);
        let output = "";
        lighthouse.stdout.on('data', (data) => {
            output += data;
        });
        lighthouse.once('close', () => tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            yield fs_jetpack_1.writeAsync(`./output/${lodash_1.kebabCase(url.replace('https://', ''))}.json`, output);
            resolve(output);
        }));
    });
};
...
axios_1.default.get(sitemapUrl)
    ...
    for (const ix in sitemap.urlset.url) {
        ...
        actions.push(() => instrumentPage(url.loc._text));
    }
